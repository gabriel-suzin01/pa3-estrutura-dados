...
def abrir_menu_relatorios(container: tk.Frame):
    ...
    opcoes = [
        (
            "1  -  Faturamento Geral entre Datas",
            lambda: _relatorio_faturamento_geral(container),
        ),
        (
            "2  -  Faturamento por Tipo de Produto",
            lambda: _relatorio_por_tipo(container),
        ),
        (
...
def _relatorio_faturamento_geral(container: tk.Frame):
    ...
    def gerar():
        ...
        pedidos = [
            p for p in arq.ler_pedidos() if val.data_no_periodo(p["data"], ini, fim)
        ]
        ...
        lbl_fat.config(
            text=f"FATURAMENTO DO PERÍODO:  R$ {total:.2f}".replace(".", ",")
        )
...
def _relatorio_por_tipo(container: tk.Frame):
    ...
    # Combobox de tipo
    tk.Label(
        fr_datas, text="Tipo:", font=ui.FONTE_NORMAL, bg=ui.COR_FUNDO, fg=ui.COR_TEXTO
    ).grid(row=0, column=4, sticky="w", padx=(0, 4))
    combo_tipo = ttk.Combobox(
        fr_datas,
        values=["Todos", "Bebida", "Lanche"],
        width=8,
        state="readonly",
        font=ui.FONTE_NORMAL,
    )
    ...
    lbl_beb = tk.Label(
        fr_totais, text="", font=ui.FONTE_NORMAL, bg=ui.COR_FUNDO, fg=ui.COR_SUCESSO
    )
    ...
    def gerar():
        ...
        lbl_tot.config(
            text=f"TOTAL GERAL   :  R$ {(total_beb+total_lan):.2f}".replace(".", ",")
        )
