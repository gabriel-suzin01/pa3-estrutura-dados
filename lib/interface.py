...
FONTE_TITULO  = ("Courier New", 14, "bold")
FONTE_NORMAL  = ("Courier New", 11)
FONTE_MENU    = ("Courier New", 12)
FONTE_CABEC   = ("Courier New", 13, "bold")
...
def frame_cabecalho(pai: tk.Widget, texto: str) -> tk.Frame:
    ...
    lbl = tk.Label(frame, text=texto, font=FONTE_CABEC,
...
def botao_menu(pai: tk.Widget, texto: str, comando, largura: int = 30) -> tk.Button:
    ...
    btn = tk.Button(
        pai, text=texto, font=FONTE_MENU, width=largura,
        bg=COR_BTN, fg=COR_TEXTO, activebackground=COR_BTN_HOVER,
        activeforeground=COR_TITULO, relief="flat", cursor="hand2",
        command=comando, pady=6
    )
...
def botao_acao(pai: tk.Widget, texto: str, comando,
               cor: str = "#45475a", largura: int = 14) -> tk.Button:
    """Cria um botão de ação menor (confirmar, cancelar etc.)."""
    btn = tk.Button(
        pai, text=texto, font=FONTE_NORMAL, width=largura,
        bg=cor, fg=COR_TEXTO, activebackground=COR_BTN_HOVER,
        activeforeground=COR_TITULO, relief="flat", cursor="hand2",
        command=comando, pady=4
    )
    return btn
...
def entrada_campo(pai: tk.Widget, largura: int = 30) -> tk.Entry:
    ...
    entrada = tk.Entry(pai, font=FONTE_NORMAL, width=largura,
                       bg=COR_PAINEL, fg=COR_TEXTO, insertbackground=COR_TEXTO,
                       relief="flat", bd=4)
    return entrada
...
def criar_treeview(pai: tk.Widget, colunas, altura: int = 12) -> ttk.Treeview:
    ...
    style.configure("Custom.Treeview",
                    ...
                    rowheight=26, fieldbackground=COR_PAINEL,
                    font=FONTE_NORMAL)
    style.configure("Custom.Treeview.Heading",
                    background=COR_BTN, foreground=COR_TITULO,
    ...
    tree = ttk.Treeview(pai, columns=ids, show="headings",
                        height=altura, style="Custom.Treeview")
